...
\begin{sloppypar}
    ...
    is to be componentized using services in which a component is indepently
    ...
    team should responsible of their product for it's full service life, the
    services are to contain the logic and communicate using a communication
    system without business logic "smart enpoints and dumb pipes", decentralized
    governance meaning that choices such as the technology to use or
    architecture are not dictated to developers, decentralized data management,
    infrastructure automation, the whole application needs to be fault tolerant
    since individual services might fail or become unavailable, and evolutionary
    design.
\end{sloppypar}
\begin{sloppypar}
    ...
    agnostic, that encapsulates data storage and retrieval and that has stable
    interface.
\end{sloppypar}
\begin{figure}[h]
...
\begin{sloppypar}    
    ...
    sanctioned by the API Gateway the performance and accessability are
    critical.
\end{sloppypar}
\begin{figure}[h]
...
\begin{sloppypar}
    ...
    following a desing patterns such as the MVC \citep{trygvemvc} this can lead
    to difficulties in the refactoring. The codebase is not split according to
...
\begin{sloppypar}
    ...
    or method invocations amongst themselves. In MSA, the messaging is based on
...
\begin{sloppypar}    
    ...
    communication model amongst the components is preserved, there would be an
...
\begin{sloppypar}
    ...
    needed to execute an action or operation or a specific authorization service
    or module can exist. An example of an web service implemented in MA is
    presented in figure \ref{fig:MA_AUTH}. The user accessing the web service
...
\begin{sloppypar}
    ...
    as one of the choices. This was found to be troublesome by \citet{nonejwt}.
    He found that many libraries did not operate in the desired way. The
    receiving party could be fooled to validate a mutated token without any
    signature with the "none" as it's algorithm. In addition to this
    ...
    the servers could be fooled in to believing that a token signed by just the
...
\begin{sloppypar}
    ...
    be handeld by the client and passed along in the requests.
\end{sloppypar}
...
\begin{sloppypar}
    ...
    ot resources to fulfill the requests in time. The circuit breaker is either
...
\section{Virtualization and Orchestration}
\begin{sloppypar}
...
\end{sloppypar}
\begin{sloppypar}
    Containers have considerably lower overhead when compared to VMs. Therefore,
    they have become the standard deployment virtualization in many of the web
    services (SOURCE). A container can be created and started easily by an
    orchestration solution such as, Docker Compose, Docker Swarm, and
    Kubernetes. All of these layers have to be correctly configured and used
    according to their specification and best practise.
\end{sloppypar}
...
\begin{sloppypar}
    ...
    a proxy. All requests and responses flow through the proxies and ar
...
\begin{sloppypar}
    ...
    communication between services. The proxies act on behalf of the service and
    all proxy-to-proxy communication can be encrypted on the transport layer
    using TLS \citep{pathtoservicemesh}. Furthermore, it facilitates the use of
    mTLS where both parties are verified.
\end{sloppypar}
\begin{sloppypar}
    Service mesh also allows for multicloud installations. 
\end{sloppypar}
...
\begin{sloppypar}
    ...
    consist of addressess, secrets, and other information that alter the
    behaviour of the system. Secrets refer to credentials, connection strings,
    other keys and similar items that are to be kept confidential. Therefore,
    the content must be stored safely and not allowed to be read or altered by
    unauthorized users.
\end{sloppypar}
...
\begin{sloppypar}
    ...
    multicloud solutions. perimeter defence -> neglect security of individual
    microservices.
\end{sloppypar}
\begin{figure}[h]
...
\begin{sloppypar}
    In this chapter flowcharts are presented that capture the main findings and
    the factors affecting decision making in and architectural switch.
\end{sloppypar}
\begin{sloppypar}
