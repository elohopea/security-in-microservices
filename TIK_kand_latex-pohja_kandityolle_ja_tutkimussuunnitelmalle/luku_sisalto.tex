...
\end{sloppypar}
\begin{sloppypar}
    This thesis is organized as follows. The next chapter compares the two
    architectures and the third chapter discusses the transition from MA to MSA.
    The fourth chapter discusses authentication and authorization. The fifth
    chapter discusses communication. The sixth chapter discusses deployment and
    production. In the seventh chapter, other relevant security concerns are
    discussed. The eighth and lastchapter in this thesis presents the
    conclusions and outlines further research topics.
\end{sloppypar}
...
\begin{sloppypar}
    ...
    business capability rather than a design pattern (such as the previously
    mentioned MVC), a team should be responsible for their product for its full
    ...
    choices such as the technology or the architecture are not dictated
...
\end{sloppypar}
\begin{sloppypar}
    Newman's (2019) definition for a microservice is a service that is
    independently deployable, is modeled around the business domain, owns the
    data that it needs to operate, communicates via a network, is technology
...
\begin{sloppypar}    
    ...
    \citet{DBLP:journals/corr/MontesiW16} present the API Gateway design pattern.
...
\begin{sloppypar}
    In MA, it is possible to implement features in such ways that the process in
...
\begin{sloppypar}
    ...
    all the information on the current operational services. All the services
    run in the same process, in which also the authentication and authentication
...
\begin{sloppypar}
    ...
    them is an authorization service that can contain the access control list.
...
\begin{sloppypar}
    ...
    to happen every time a user tries to access functionality with access
...
\end{figure}
\begin{sloppypar}
    To limit excessive communication, a token-based authentication and
    authorization scheme can be used. A simplified process flow is presented in
    figure \ref{fig:TOKEN_AUTH}. A user enters credentials to login, and with
    these credentials, an authentication and authorization service grants the
    user a token. This token is used to access the services in the system. The
    services in question trust the issuer of the token and verify the token. If
    the token is accepted and access can be granted, the request is serviced
    without any communication between the service and the authorization service.
    A specific token-based authentication protocol is discussed in the next
    section.
\end{sloppypar}
...
\end{figure}
\begin{sloppypar}
    An example of the OIDC message flow is presented in figure
    \ref{fig:oidcflow}. The process flow is the following: the client
    application requests authentication from the identity provider, the end-user
    is authenticated and authorization is obtained, the identity provider
    responds to the initial authentication request by sending an ID token and a
    possible access token, the client application can request the end-user
    claims from the authorization server with the access token, and finally the
    authorization service returns the claims to the client application. The
    message flow is similar to the OAuth 2.0, and the token sent to the client
    application is a JavaScript Object Notation Web Token (JWT).
\end{sloppypar}

\subsection{JWT}
\begin{sloppypar}
    JWT is a format to represent claims. It consists of base64 encoded,
    dot-delimited strings, which can be easily carried in the HTTP request or
...
\begin{sloppypar}
    ...
    solutions. One of the solutions discussed in the paper was an opaque access
    token for the client to API-Gateway communication, which is mapped to JWT in
    the API Gateway. The JWT would be used in all communication as the means to
    authenticate and authorize the user. The main problem the opaque token is
    said to solve is the logout problem. In comparison, a bearer token is valid
    until its expiration, and the issuer cannot invalidate the token. The opaque
    token flow is presented in figure \ref{fig:OPAQUE_TOKEN_API_GATEWAY}.
\end{sloppypar}
\begin{sloppypar}
    ...
    the services directly. Furthermore, the opaque token can be revoked by
    simply removing it from the storage. The internal JWT can alleviate some of
    the issues that arise when an application is transformed from MA to MSA.
    Firstly, token theft, though still possible, is limited to the time the
...
\begin{sloppypar}
    ...
    Remote Procedure Calls (RPC). For a more complete list view, see
    \citet{secchalmsa}. In this thesis, the REST API will be examined further.
\end{sloppypar}
...
\begin{sloppypar}
    ...
    depicted in figure \ref{fig:orchestration_constituents}. Scaling refers to
    ...
    new version that needs to be rolled into production. The upgrade service is
    responsible for doing this. Service discovery is a service that is used to
    locate running services and to which new services register. If a service
    becomes nonoperational, it might not be able to send the orchestrator any
    message on this erroneous behavior. Therefore, the orchestrator can have a
    health check service. This service can periodically send a message to a
    service and, if no response is not received within a reasonable time, the
    service is deemed nonoperational and a new one is created by the scheduler.
    This service creates the individual pods or containers according to the
    system settings. The last orchestration part is the organizational
    primitives. These are used, e.g., to label pods or containers with matching
    business domain names. This is to make the administration and setup work
    easier \citep{containernetworking}.
\end{sloppypar}
...
\begin{sloppypar}
    ...
    administrators, a Policy Information Point (PIP), and a Policy Decision
    Point (PDP), and collects metrics of the behavior and actions in the system.
    The data moves in the data-plane. Each microservice is accompanied by a
    proxy. All requests and responses flow through the proxies and are
...
\begin{sloppypar}
    ...
    Linkerd, and Consul Connect. As was the case with authentication and
    security in general, it is not advisable to implement security-critical
    features if there exists an off-the-shelf alternative.
\end{sloppypar}
\begin{sloppypar}
    ...
    on the transport layer with TLS \citep{pathtoservicemesh}. Furthermore, the
    service mesh facilitates the use of mTLS where both parties are verified. It
...
\begin{sloppypar}
    ...
    might be applicable, but for most web services, it is not feasible.
\end{sloppypar}
\begin{figure}[h]
