...
\begin{sloppypar}
    ...
    near future. This is furthermore amplified using agile software
...
\begin{sloppypar}
    ...
    team should be responsible of their product for its full service life, the
...
\begin{sloppypar}
    ...
    is because that every microservice accessible to the client can also be
    accessed or contacted by other more malicious parties in the same network.
    The network in the case of web services is the internet. The attack surface
...
\begin{sloppypar}
    ...
    actual splitting of the monolith can be carried out in various ways, one of
    which is Domain Driven Design \citep{evans2003}. The selection of boundaries
    for the services is critical. If this is done incorrectly all the affected
...
\begin{sloppypar}
    ...
    calls. Function calls entail a stack frame creation in the call stack,
    execution of the function code, and finally popping the stack frame and
    returning the result. Compilers can optimize the code further and inline the
    function calls to eliminate the stack frame creation and the following
    procedures.
\end{sloppypar}
\begin{sloppypar}
...
\begin{sloppypar}
    ...
    should be changed to consider the change in communication path. If the
...
\begin{sloppypar}
    ...
    to be secure the defending developers must find all the bugs whereas
    the attacker must find only one. Therefore, to make systems more secure
...
\begin{sloppypar}
    ...
    token is transmitted to the user via the network. This communication in both
    MA and MSA, should be encrypted in a way that none of the actors in the
...
\begin{sloppypar}
    ...
    user must be available for the web service for verification. When using
    MSA, the service should own its own data. Whenever information is available
    it is a target for thieves and hackers. The services in MSA are to be
    individually deployable and the service scalable. Authentication service
    implementation must take this into account. The service must adhere to
...
\begin{sloppypar}
    ...
    already existing framework or another entity providing the
...
\subsection{OIDC}
\begin{sloppypar}
    OIDC is an identity layer to accompany OAuth 2.0 authorization
...
\begin{sloppypar}
    ...
    claims to the client application. The presented flow is like the OAuth
...
\begin{sloppypar}
    ...
    contents are key value pairs, and the token may or may not be signed and
...
\begin{sloppypar}
    ...
    token is valid until expiration and the client cannot invalidate the token.
...
\begin{sloppypar}
    ...
    still must be the public keys in the server for signature verification.
\end{sloppypar}
...
\begin{sloppypar}
    ...
    organization or individuals themselves. The software can be installed on
...
\begin{sloppypar}
    ...
    complex solution than in MA. The added layers such as the K8s, must be
    ...
    microservices are implemented or even planned the security should be considered as early as possible. Implementing security later in the
    project or as an afterthought can be more expensive and very difficult.
\end{sloppypar}
...
\begin{sloppypar}
    ...
    install operating system or nonessential software a new. Only the
...
\begin{sloppypar}
    ...
    All these tools must be correctly configured and used according to
...
\begin{sloppypar}
    ...
    carried out. If a service is to become nonoperational it might not be able
    ...
    within reasonable time frame the service is deemed nonoperational and a
    ...
    orchestration part is the organizational primitives. These are used to
...
\begin{sloppypar}
    ...
    in all the services. The service mesh can provide a certificate authority
...
\begin{sloppypar}
    ...
    as one of the choices. This was found to be troublesome by \citet{nonejwt}. He found that
...
\begin{sloppypar}
    ...
    services should contain or have access to its own data i.e. database. In MA
    ...
    actions fail permanently the changes that have already been made need to be
    undone. In addition to this, a HTTP is asynchronous, and the execution order
...
\begin{sloppypar}
    ...
    or responds. Malicious attempts in an MA are more easily screened by
    performing security audits and by peer reviewing the code. In an MSA the
    knowledge of a single service and its inner workings are shared by a more
...
\begin{sloppypar}
    ...
    audits and by peer reviewing the code. In an MSA the knowledge of a single
    service and its inner workings are shared by a more limited number of
...
\begin{sloppypar}
    ...
    In this approach there exists a command center that is always to be manned and as such be able to respond to the encountered threats. In some
    instances, this might be applicable, but for all web services this is not be
    feasible.
\end{sloppypar}
\begin{figure}[h]
...
\begin{sloppypar}
    ...
    cases, these tools must be  implemented by in house developers and thus more
    costs are incurred upfront and in the upkeep of the system. In addition
...
\begin{sloppypar}
    ...
    potentially exposed attack surface. Security can be thought of as being as
    good as its weakest link. In general, an MSA deployment has multiple layers
    which all must be consistent and correct. One example is the
    configuration of the operating system on the server running the
    virtualization environment. All the layers from the server hardware to
    the handling of errors in the actual code must be of ample quality to
...
\end{sloppypar}
\begin{sloppypar}
    Security must be considered right from the beginning of the
...
\begin{sloppypar}
    ...
    can act as a PEP and allow or disallow a request. Also, if a service mesh
...
\begin{sloppypar}
    ...
    architecture change is feasible in a case. This can be the case
